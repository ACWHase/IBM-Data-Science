...
def get_pie_chart(entered_site):
    if entered_site == 'ALL':
        ...
        fig = px.pie(filtered_df, values='Unnamed: 0', names='class', title='Total Successful Launches at chosen site {}'.format(entered_site))
    return fig
...
def get_scatter_plot(entered_site,payload_range):
    scatter_df = spacex_df[(spacex_df['Payload Mass (kg)']>=int(payload_range[0])) & (spacex_df['Payload Mass (kg)']<= int(payload_range[1]))]
    if entered_site == 'ALL':	
        ...
        fig = px.scatter(filtered_scatter_df, x='Payload Mass (kg)', y='class', color='Booster Version Category',title='Correlation Between Payload and Success at chosen site {}'.format(entered_site))
    return fig
